...
func query7(db *sql.DB, params []string, st string)(*sql.Rows) {
  rows, err := db.Query(st, params[0], params[1], params[2], params[3], params[4], params[5], params[6])
  if (err != nil) {
    log.Fatal(err)
  }
  return rows
...
func query8(db *sql.DB, params []string, st string)(*sql.Rows) {
  rows, err := db.Query(st, params[0], params[1], params[2], params[3], params[4], params[5], params[6], params[7])
  if (err != nil) {
    log.Fatal(err)
  }
  return rows
